/**
 * Forked from https://github.com/mdit-plugins/mdit-plugins/blob/main/packages/footnote/src/plugin.ts
 */
...
import { type FootNoteEnv, type FootNoteToken } from "./markdownTypes";
...
function getIDSuffix(tokens: FootNoteToken[], index: number): string {
  return tokens[index].meta.subId > 0 ? `:${tokens[index].meta.subId}` : "";
}
...
function footnoteTail(state: FootNoteStateCore): boolean {
