...
const needUpdateDateOrReadingTime = (frontmatter:any) => {
  ...
  if (frontmatter?.update && dayjs(frontmatter?.date).isValid()) {
    return true
  }

  if (frontmatter?.date === "now" || frontmatter?.update === 'now') {
    return true
  }
  return false
};
...
export const pageInfo: PluginSimple = (md) => {
  ...
  md.renderer.rules.text = (...args) => {
    ...
    if (
      !args[3] ||
      !args[3].id ||
      idMapper.get(args[3].id) || !needUpdateDateOrReadingTime(args[3].frontmatter)
    )
      return `${text(...args)}`;
...
function updateFrontmatterDate(frontmatter: aRef<string>) {
  ...
  if (!frontmatter.value.includes(`update: now`)) return false;
