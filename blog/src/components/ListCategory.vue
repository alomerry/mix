...
const routes: Post[] = router.getRoutes()
  ...
  .filter(i => !i.path.endsWith('.html') && (i.meta.frontmatter.type || props.type || '// empty').split('+').some(i => i.includes(props.type)))
  .map(i => ({
  ...
  <ul>
    ...
    <template v-for="route, idx in posts" :key="route.path">
      <div
        ...
        select-none relative h16 pointer-events-none slide-enter
        :style="{
          '--enter-stage': idx - 2,
          '--enter-step': '60ms',
        }"
      >
        <span text-8em color-transparent absolute left--3rem top--1rem font-bold text-stroke-2 text-stroke-hex-aaa op25>{{ getGroupName(route) }}</span>
      </div>
      <div
        class="slide-enter"
        :style="{
      ...
      >
        ...
        >
          ...
          <div v-if="route.desc" op50 text-sm hidden mt--2 md:block>
            {{ route.desc }}
          </div>
